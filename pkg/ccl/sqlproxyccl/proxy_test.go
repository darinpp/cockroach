...
`
	...
	handler, err := NewProxyHandler(context.Background(), *opts)
	require.NoError(t, err)
...
func TestLongDBName(t *testing.T) {
	...
	defer testutils.HookGlobal(&BackendDial,
		func(_ *pgproto3.StartupMessage, _ string, _ *tls.Config) (net.Conn, error) {
			return nil, NewErrorf(CodeParamsRoutingFailed, "boom")
		})()
	defer testutils.HookGlobal(&SendErrToClient, func(conn net.Conn, err error) {
	...
	s, addr, done := setupTestProxyWithCerts(t, &ProxyOptions{})
	defer done()
